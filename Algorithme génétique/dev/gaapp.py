...
#                        |_|   |_|    
# ------------------------------------
# Application de résolution de problèmes par algorithmes génétiques
# ------------------------------------
...
#    |___/                                                                
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Classe permettant de faire le lien entre 2 bibliothèques qui ne sont pas 
    ...
    def update(self):
        if self._ga_adapter.genetic_algorithm.current_epoch == 0:
            ...
            for i in range(self._ga_adapter.problem_definition.domains.dimension):
                solution_info += f'\n    - {self._ga_adapter.problem_definition.domains.names[i]} : {self._ga_adapter.genetic_algorithm.history.best_solution[i]:0.8f}'
...
Fitness : 
    - best    : {self._ga_adapter.genetic_algorithm.history.best_fitness:>16.12f}
    - worst   : {self._ga_adapter.genetic_algorithm.history.worst_fitness:>16.12f}
    - average : {self._ga_adapter.genetic_algorithm.history.average_fitness:>16.12f}
    - std dev : {self._ga_adapter.genetic_algorithm.history.standard_deviation_fitness:>16.12f}
    - median  : {self._ga_adapter.genetic_algorithm.history.median_fitness:>16.12f}
{solution_info}'''
...
class QGAParametersWidget(QGroupBox):
    ...
    def __init__(self, ga_adapter, parent=None):
        ...
        self._initialization_strategy_combo = QComboBox()
        self._initialization_strategy_combo.enabled = False
        self._selection_strategy_combo = QComboBox()
        self._crossover_strategy_combo = QComboBox()
        self._mutation_strategy_combo = QComboBox()
        self._initialization_strategy_combo.add_items(('to do',))
        ...
        initialization_strategy_title = create_title('Initialization strategy', 'Define the initialization algorithm used for the genetic algorithm.')
        ...
        layout.add_row(initialization_strategy_title, self._initialization_strategy_combo)
