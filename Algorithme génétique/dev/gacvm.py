...
def solve_linear_regression_problem():
    from math import floor, log10
    ...
    # Définition du problème.
    # Soit la droite y = ax + b, on cherche à trouver les valeurs de a et b
    a = -2.0
    b = 7.5
    y_noise = 1.5
    x_range = 10.0
    x_span = np.array([-x_range, x_range])
    n_points = 100
    rng = np.random.default_rng()
    x = rng.uniform(x_span[0], x_span[1], n_points)
    y = a * x + b + rng.normal(0.0, y_noise, n_points)
    y_span = np.array([np.min(y), np.max(y)])
    
    ascii_art =np.full((40, 100), ' ', np.str_)
    for value_x, value_y in zip(x, y):
        x_image = int((value_x - x_span[0]) / (x_span[1] - x_span[0] + 1) * ascii_art.shape[1])
        y_image = int((value_y - y_span[0]) / (y_span[1] - y_span[0] + 1) * ascii_art.shape[0])
        
        ascii_art[y_image, x_image] = '*'
    ...
    intervals = np.array([[-10, 10],
                          [-10, 10]], dtype=np.float64)
    axis_names = ('a', 
                  'b')
    
    domains = Domains(intervals, axis_names)
    ...
    # tableau 1D de nombres réels, et retourne un nombre réel représentant 
    # la performance relative de la solution donnée, le chromosome.
    def fitness_delta_y(chromosome):
        estimated_a = chromosome[0]
        estimated_b = chromosome[1]
        
        cumulated_error = 0.0
        for value_x, value_y in zip(x, y):
            estimated_y = estimated_a * value_x + estimated_b
            error = estimated_y - value_y
            cumulated_error += error ** 2
        
        return 1 / cumulated_error

    def fitness_delta_distance(chromosome):
        estimated_a = chromosome[0]
        estimated_b = chromosome[1]
        
        cumulated_distance = 0.0
        for value_x, value_y in zip(x, y):
            num = abs(estimated_a * value_x - value_y + estimated_b)
            den = (estimated_a ** 2 + 1) ** 0.5
            perp_distance = num / den
            cumulated_distance += perp_distance
        
        return 1 / cumulated_distance
    ...
    class DisplayObserver(Observer):
        def update(self, engine):
            ...
            progress_absolute = f'Epoch {engine.current_epoch:0{number_size}} of {last_epoch}'
            progress_relative = f'[{progress_char}{progress_filling}] {advancement * 100:0.1f}%'
            print(f'\r{progress_absolute} {progress_relative} : a, b = ({engine.history.best_solution[0]:0.12f}, {engine.history.best_solution[1]:0.12f})', end='')

    # Résolution du problème.
    # Instanciation du moteur de résolution, configuration des paramètres.
    ga = GeneticAlgorithm()
    ga.parameters = parameters
    ga.problem_definition = ProblemDefinition(domains, fitness_delta_distance)
    ga.add_observer(DisplayObserver()) # Ajout de l'observateur, optionnel
    
    # Lancement de l'évolution.
    print('Resolving linear regression problem...')
    ga.evolve() 
    #  ^^^^^^ Cette fonction est bloquante et ne retourne que lorsque 
    #         l'évolution est terminée.
    #         Malgré tout, une rétroaction est donnée par l'observateur.
    ...
    print()
    print('... linear regression problem solved!')    
...
if __name__ == '__main__':
    # solve_open_box_problem()
    solve_linear_regression_problem()
